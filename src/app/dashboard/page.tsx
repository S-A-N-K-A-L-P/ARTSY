...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-40 transition-colors duration-500">
      {/* Header Tabs */}
      <div className="flex items-center gap-8 mb-12 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
        <button 
          onClick={() => setActiveTab('home')}
          className={cn(
            "pb-4 text-[10px] font-black uppercase tracking-[0.3em] transition-all relative translate-y-[1px]",
            activeTab === 'home' ? "opacity-100" : "opacity-30 hover:opacity-60"
          )}
          style={{ 
            color: activeTab === 'home' ? 'var(--accent)' : 'var(--text-primary)',
        ...
        <button 
          ...
          className={cn(
            "pb-4 text-[10px] font-black uppercase tracking-[0.3em] transition-all relative translate-y-[1px]",
            activeTab === 'manage' ? "opacity-100" : "opacity-30 hover:opacity-60"
          )}
          style={{ 
            color: activeTab === 'manage' ? 'var(--accent)' : 'var(--text-primary)',
          ...
          >
             <div className="mb-12 text-center max-w-sm">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--accent-soft)] text-[var(--accent)] text-[9px] font-black uppercase tracking-widest mb-4">
                  <Sparkles size={10} /> Live Selection
                </div>
                <h2 className="text-5xl font-bold tracking-tighter mb-4 italic" style={{ color: 'var(--text-primary)' }}>The Void</h2>
                <p className="opacity-40 text-[10px] font-black uppercase tracking-widest leading-relaxed">Discovery of artifacts through the lens of aesthetic intent</p>
             </div>
             <FeedReels />
          </motion.div>
        ) : (
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-12 flex flex-col justify-end translate-y-6 group-hover:translate-y-0 transition-all duration-700">
        ...
        <div className="mt-10 flex items-center gap-2 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all delay-100">
          <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.4em] text-white">
            Access Artifacts <ArrowRight size={16} className="text-[var(--accent)]" />
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-10 right-10 flex flex-col gap-4">
        ...
        <div className="w-14 h-14 rounded-3xl backdrop-blur-xl bg-black/20 border border-white/10 flex items-center justify-center text-white/40 opacity-0 group-hover:opacity-100 transition-all delay-75">
