  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between px-1">
        <span className="text-[10px] font-black uppercase tracking-[0.4em] text-text-muted">Logistics Flow</span>
        {date && (
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
            <span className="text-[10px] font-black text-text uppercase tracking-[0.2em] italic">Est: {date}</span>
          </div>
        )}
      </div>
      ...
      <div className="relative flex items-center justify-between w-full px-4">
        {/* Connection Line */}
        <div className="absolute top-6 left-10 right-10 h-px bg-elevated z-0">
           <motion.div 
             initial={{ width: 0 }}
           ...
           <div key={step.key} className="flex flex-col items-center gap-4 relative z-10">
              <motion.div 
                 ...
                 className={cn(
                    ...
                    i <= currentIndex ? "border-accent text-text shadow-xl shadow-black/10" : "bg-card text-text-muted border-line"
                 )}
              >
