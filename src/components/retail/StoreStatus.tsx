...
export const StoreStatus = ({ isOpen, message, className, hours }: StoreStatusProps) => {
  return (
    <motion.div
      ...
      className={cn("flex items-center gap-4 bg-elevated px-6 py-2.5 rounded-full border border-line shadow-sm", className)}
    >
      <div className="relative">
         <motion.div
           animate={{ scale: [1, 1.5, 1], opacity: [0.1, 0, 0.1] }}
           transition={{ duration: 2, repeat: Infinity }}
           className={cn("absolute inset-0 rounded-full", isOpen ? "bg-emerald-500" : "bg-text-muted")}
         />
         <div className={cn("w-2 h-2 rounded-full relative z-10", isOpen ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "bg-text-muted")} />
      </div>
      <div>
