    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        {items.slice(0, 4).map((item, i) => (
          ...
          >
            <div className="aspect-[4/5] rounded-2xl overflow-hidden bg-elevated border border-line">
               <img src={item.image} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
            </div>
            <div className="px-1">
               <h4 className="text-[11px] font-bold text-text truncate">{item.title}</h4>
               <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-0.5">₹{item.price}</p>
            </div>
          </motion.div>
        ))}
      </div>
