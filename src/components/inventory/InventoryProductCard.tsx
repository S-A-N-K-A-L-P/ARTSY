...
export function InventoryProductCard({ item, onEdit, onDelete }: InventoryProductCardProps) {
  ...
  return (
    <motion.div 
      ...
      className="group bg-card rounded-[32px] border border-line overflow-hidden shadow-sm hover:shadow-[0_40px_100px_rgba(0,0,0,0.06)] transition-all duration-700"
    >
      {/* High-Dominance Image Container (80% weight) */}
      <div className="relative h-64 overflow-hidden bg-elevated group-hover:scale-[0.98] transition-transform duration-700 m-2 rounded-[24px]">
        <img 
        ...
        <div className="absolute inset-x-4 bottom-4 flex gap-2 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
            <button 
                ...
                className="flex-1 h-12 bg-card text-text rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-2xl hover:bg-accent hover:text-white transition-all"
            >
                <Edit size={14} /> Edit
            </button>
            <button 
                onClick={() => onDelete(item._id)}
                className="w-12 h-12 bg-card/20 backdrop-blur-xl border border-white/30 text-white rounded-xl flex items-center justify-center hover:bg-red-500 hover:border-red-500 transition-all"
            >
                <Trash2 size={16} />
            </button>
        </div>
      ...
      <div className="p-6 pt-4">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <h3 className="text-base font-black tracking-tighter text-text line-clamp-1 leading-tight group-hover:text-text-secondary transition-colors italic">
              {item.title}
            </h3>
          ...
          <p className="text-lg font-black tracking-tighter text-text italic">
        ...
        <div className="flex items-center justify-between mt-6 pt-6 border-t border-line">
            <div className="flex items-center gap-2">
                ...
                <span className="text-[8px] font-black uppercase tracking-widest text-text-muted">Synced Live</span>
            </div>
            <button 
