...
export const AttributeBadge = ({ label, value }: { label: string, value: string }) => (
  <div className="px-4 py-2 rounded-xl bg-card/5 border border-line flex flex-col">
     <span className="text-[8px] font-bold uppercase tracking-widest text-text/20">{label}</span>
     <span className="text-xs font-bold text-text/80">{value}</span>
  </div>
);
...
export const AddToCartButtonMobile = ({ price, currency, onClick }: { price: number, currency: string, onClick?: () => void }) => (
  <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black to-transparent z-[60]">
     <button 
       onClick={onClick}
       className="w-full h-16 rounded-[24px] bg-card text-text font-extrabold flex items-center justify-between px-8 shadow-2xl shadow-white/10"
     >
...
export const DescriptionBlockMobile = ({ text }: { text: string }) => (
  <div className="space-y-4">
     <h3 className="text-xs font-bold uppercase tracking-[0.4em] text-text/20">The Story</h3>
     <p className="text-sm font-medium leading-relaxed text-text/60">
        {text}
     </p>
  </div>
);
...
export const AestheticThemeBadge = ({ theme }: { theme: string }) => (
  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-card/5 border border-line">
...
export const PriceFilterMobile = () => (
   <div className="p-6 rounded-3xl bg-card/5 border border-line">
      <h4 className="text-[10px] font-bold uppercase tracking-[0.4em] opacity-20 mb-4">Price Range</h4>
      <div className="flex items-center gap-4">
         <div className="flex-1 h-1 bg-card/10 rounded-full relative">
            <div className="absolute left-1/4 right-1/4 h-full bg-card" />
         </div>
      </div>
   </div>
);
  ...
  <div className="flex items-center justify-between px-10 py-6 mb-8">
     {[1, 2, 3].map(s => (
        <div key={s} className="flex items-center gap-2">
           <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold", s <= currentStep ? "bg-card text-text" : "bg-card/5 text-text/20")}>{s}</div>
           {s < 3 && <div className={cn("w-8 h-px", s < currentStep ? "bg-card" : "bg-card/10")} />}
        </div>
     ))}
  </div>
);
...
export const EmptyStateMobile = () => (
  <div className="py-20 flex flex-col items-center justify-center text-center px-10">
     <p className="text-text/20 text-xs font-bold uppercase tracking-widest">No items in this collection</p>
  </div>
);
