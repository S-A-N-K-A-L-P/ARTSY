...
export const AestheticSettingsModalDesktop = () => {
  ...
  return (
   <div className="p-10 rounded-[48px] bg-card/5 border border-line flex flex-col gap-8">
         ...
         {(Object.keys(themes) as ThemeName[]).map(t => (
            <button 
               ...
               className={cn(
                  ...
                  aesthetic === t ? "bg-card text-text border-line" : "bg-card/5 text-text/40 border-line hover:border-line"
               )}
            >
               {t}
      ...
      <div className="space-y-4 pt-4 border-t border-line">
         {['Override Default Fonts', 'Enable Grain FX', 'Force Dark Perspective'].map(p => (
            <div key={p} className="flex justify-between items-center">
               ...
               <div className="w-10 h-5 rounded-full bg-card/10 p-1 flex justify-end"><div className="w-3 h-3 rounded-full bg-card" /></div>
            </div>
         ))}
      </div>
   </div>
...
export const PageCustomizerSidebar = ({ page, onUpdate }: any) => (
  <div className="w-96 space-y-12 p-10 bg-card/5 border-l border-line h-[calc(100vh-6rem)] sticky top-24 overflow-y-auto no-scrollbar">
     ...
     <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-2">
           {['Serif', 'Mono', 'Sans', 'Display'].map(f => (
              <button key={f} className="h-12 rounded-xl bg-card/5 border border-line text-xs font-bold hover:border-line transition-all">{f}</button>
           ))}
        </div>
     </div>
     <div className="space-y-6">
...
export const BulkActionToolbar = () => (
    <div className="fixed bottom-10 left-1/2 -translate-x-1/2 px-8 py-4 rounded-3xl bg-card/10 backdrop-blur-2xl border border-white/20 shadow-2xl flex items-center gap-8 z-[100]">
       <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest pr-8 border-r border-line">
          <span className="w-6 h-6 rounded-lg bg-card/20 flex items-center justify-center">3</span> Selected
       </div>
