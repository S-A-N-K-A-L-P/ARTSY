...
export const EmptyStateCreator = () => (
  <div className="col-span-full py-40 border-2 border-dashed border-line rounded-[60px] flex flex-col items-center justify-center p-10 text-center">
     <div className="w-20 h-20 rounded-full bg-card/5 flex items-center justify-center mb-8">
        <Layers size={32} className="opacity-20" />
     </div>
     <h3 className="text-2xl font-bold tracking-tight mb-4">No aesthetic items found</h3>
     <p className="text-text/40 max-w-xs mb-10 font-medium">Start building your storefront by adding your first curated piece.</p>
     <button className="h-14 px-10 rounded-2xl bg-card text-text font-extrabold uppercase tracking-widest text-xs">Create Item</button>
  </div>
);

// 8. CheckoutSummaryPanel
export const CheckoutSummaryPanel = ({ cartItems, total }: { cartItems: CartItem[], total: string }) => (
  <div className="w-96 sticky top-24 p-10 rounded-[48px] bg-card text-text shadow-2xl space-y-8">
     ...
     <div className="pt-8 border-t border-black/10 space-y-4">
        ...
        <button className="w-full h-16 rounded-2xl bg-accent text-bg font-bold uppercase tracking-[0.2em] text-xs active:scale-[0.98] transition-all">Secure Checkout</button>
     </div>
  </div>
);
...
export const BulkAssetUploaderDesktop = () => (
   <div className="col-span-full h-80 rounded-[60px] bg-card/5 border-2 border-dashed border-line flex flex-col items-center justify-center p-10 text-center group hover:bg-card/[0.07] transition-all">
      ...
      <p className="text-text/40 max-w-xs font-medium italic">Drop up to 50 assets to auto-generate gallery pieces.</p>
   </div>
);

// 14. DragDropOrganizerDesktop
export const DragDropOrganizerDesktop = ({ items }: any) => (
  <div className="p-10 rounded-[60px] border-2 border-dashed border-line flex flex-col items-center justify-center group hover:border-line transition-all cursor-move">
