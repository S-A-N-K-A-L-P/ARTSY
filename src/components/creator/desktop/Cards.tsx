...
export const HoverPreviewCard = ({ item, onClick }: { item: AestheticItem, onClick?: () => void }) => (
  <motion.div 
    ...
    onClick={onClick}
    className="group relative h-[450px] rounded-[48px] overflow-hidden bg-card/5 border border-line cursor-pointer"
  >
    ...
    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-10 flex flex-col justify-end translate-y-4 group-hover:translate-y-0 transition-all duration-500">
       <div className="flex items-center gap-2 mb-3 opacity-0 group-hover:opacity-100 transition-all">
          <span className="px-3 py-1 rounded-full bg-card/10 backdrop-blur-md text-[9px] font-bold uppercase tracking-widest">{item.category}</span>
          <span className="px-3 py-1 rounded-full bg-card/10 backdrop-blur-md text-[9px] font-bold uppercase tracking-widest">{item.aesthetic}</span>
       </div>
       <h3 className="text-3xl font-bold tracking-tighter mb-2">{item.title}</h3>
       <p className="text-sm font-bold italic tracking-tight text-text/60 mb-6">{item.currency} {item.price?.toLocaleString()}</p>
       
       <div className="h-1 bg-card/20 rounded-full overflow-hidden">
          <motion.div initial={{ x: '-100%' }} whileHover={{ x: '0%' }} className="h-full bg-card" />
       </div>
    </div>
    <div className="absolute top-8 right-8 w-12 h-12 rounded-full bg-black/50 backdrop-blur-xl border border-line flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all scale-75 group-hover:scale-100">
       <Maximize2 size={18} />
    </div>
  </motion.div>
);
...
export const UserProfileMiniDesktop = ({ user }: { user?: UserProfile }) => (
  <div className="flex items-center gap-6 p-6 rounded-[32px] bg-card/5 border border-line">
     <div className="w-14 h-14 rounded-2xl bg-card/10" />
     ...
     <button className="w-12 h-12 rounded-2xl bg-card text-text flex items-center justify-center"><Plus size={18} /></button>
  </div>
);
...
export const AttributeDataGrid = ({ attributes }: { attributes?: Record<string, string> }) => (
  <div className="grid grid-cols-1 gap-4">
     {Object.entries(attributes || {}).map(([k, v]) => (
        <div key={k} className="flex items-center justify-between py-4 border-b border-line">
           <span className="text-xs font-bold uppercase tracking-[0.3em] text-text/20">{k}</span>
...
export const ItemDetailSidebarDesktop = ({ item }: { item: AestheticItem }) => (
  <div className="space-y-12">
     <div className="flex items-center gap-4">
        <div className="w-12 h-12 rounded-full bg-card/5 border border-line" />
        <p className="font-bold tracking-tight opacity-40 italic">Managed by {item.owner || "Trustee"}</p>
     </div>
     <div className="p-10 rounded-[40px] border border-line space-y-4">
