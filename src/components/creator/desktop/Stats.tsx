  ...
  <div className="grid grid-cols-4 gap-6">
     {[
       ...
       { label: 'Page Rank', value: '#12', icon: Award, color: 'text-purple-400' }
        ...
        <div key={s.label} className="p-8 rounded-[40px] bg-card/5 border border-line">
           <div className={cn("w-12 h-12 rounded-2xl bg-card/5 flex items-center justify-center mb-6", s.color)}>
              <s.icon size={24} />
           </div>
...
export const CreatorInsightCard = ({ label, chart }: { label: string, chart?: any }) => (
  <div className="p-10 rounded-[48px] bg-card/5 border border-line space-y-6">
     ...
     <div className="h-32 w-full flex items-end gap-1">
        {[40, 70, 45, 90, 65, 80, 55].map((h, i) => (
           <div key={i} className="flex-1 bg-card/10 rounded-t-lg transition-all hover:bg-card/40" style={{ height: `${h}%` }} />
        ))}
     </div>
  </div>
);
   ...
   <div className="space-y-6">
      ...
      <div className="grid grid-cols-12 gap-1">
         {Array.from({ length: 48 }).map((_, i) => (
            <div key={i} className="aspect-square rounded-sm bg-card" style={{ opacity: Math.random() * 0.3 }} />
         ))}
      </div>
   </div>
);
