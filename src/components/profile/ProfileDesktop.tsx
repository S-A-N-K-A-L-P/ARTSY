  ...
  return (
    <div className="space-y-32 pb-40">
      <AestheticRenderer 
        ...
        props={{ user: creator }} 
        fallback={
          <div className="h-64 rounded-[60px] bg-card/5 border border-line flex items-end p-16 mx-12 mt-12 shadow-2xl">
      ...
      {featuredPage && (
        <section className="px-12 md:px-20 max-w-[1700px] mx-auto">
           <motion.div 
             ...
             className="relative h-[700px] rounded-[80px] overflow-hidden border border-line shadow-[0_0_100px_rgba(0,0,0,0.5)] group"
           >
              <img 
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black via-black/40 to-transparent p-24 flex flex-col justify-center">
                 <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="space-y-8 max-w-2xl">
                    <span className="px-6 py-2 rounded-full bg-amber-500 text-text text-[10px] font-black uppercase tracking-[0.4em]">Featured Masterpiece</span>
                    <h2 className="text-8xl font-black tracking-tighter italic uppercase leading-none">{featuredPage.name}</h2>
                    <p className="text-xl text-text/60 italic leading-relaxed">{featuredPage.description}</p>
                    <div className="flex gap-4">
                       <button onClick={() => router.push(`/dashboard/${creator.username}/${featuredPage.slug}`)} className="h-20 px-12 rounded-full bg-card text-text font-black uppercase tracking-[0.3em] text-[10px] flex items-center gap-4 hover:scale-105 transition-all">
              ...
              <div className="absolute top-12 right-12 flex flex-col gap-4">
                 {[
                   ...
                   <motion.div 
                     ...
                     className="px-8 py-5 rounded-[32px] bg-card/5 backdrop-blur-3xl border border-line flex items-center gap-6 min-w-[200px]"
                   >
      ...
      <section className="px-12 md:px-20 max-w-[1700px] mx-auto">
        <div className="flex gap-20">
          ...
          <div className="flex-1 space-y-16">
            <div className="flex items-center justify-between border-b border-line pb-12">
               ...
               <div className="flex items-center gap-6">
                  ...
                  <div className="w-px h-12 bg-card/5 hidden xl:block" />
                  <div className="flex -space-x-4">
                     {pages.slice(0, 5).map((p, i) => (
                       <div key={i} className="w-14 h-14 rounded-full border-4 border-black overflow-hidden bg-card/10 shadow-2xl">
            ...
            <MasonryGridDesktop>
              {pages.map((page) => (
                <motion.div 
                  ...
                  className="group relative h-[550px] rounded-[72px] overflow-hidden bg-card/5 border border-line hover:border-line transition-all shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] cursor-pointer"
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-16 flex flex-col justify-end translate-y-8 group-hover:translate-y-0 transition-all duration-700">
                     <div className="flex gap-3 mb-6">
                        <span className="px-5 py-2 rounded-full bg-card/10 backdrop-blur-3xl text-[9px] font-black uppercase tracking-[0.2em] border border-line">{page.type}</span>
                     ...
                     <p className="text-sm text-text/30 mt-8 line-clamp-2 leading-relaxed italic tracking-tight font-medium">{page.description}</p>
                     ...
                     <div className="mt-12 flex items-center justify-between opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all delay-100">
                        <span className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.5em] text-text">
                           DECODE SPACE <ArrowRight size={14} className="group-hover:translate-x-2 transition-transform duration-500" />
                        </span>
                        <div className="w-12 h-12 rounded-full bg-card/10 flex items-center justify-center backdrop-blur-xl border border-line">
